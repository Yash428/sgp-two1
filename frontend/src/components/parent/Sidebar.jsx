...
import { PARENT_DASHBOARD_SIDEBAR_LINKS ,PARENT_DASHBOARD_SIDEBAR_BOTTOM_LINKS} from '../lib/Navigation';
...
function Sidebar() {
    return (
        ...
        <div className='flex-1  py-8 flex flex-col gap-0.5 text-neutral-100'>{
            PARENT_DASHBOARD_SIDEBAR_LINKS.map((item) => (
                <SidebarLink key={item.key} item={item} />
            ))
        }</div>
        <div>
            {PARENT_DASHBOARD_SIDEBAR_BOTTOM_LINKS.map((item)=>(
                <SidebarLink key={item.key} item={item} />
            ))}
